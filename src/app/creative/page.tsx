...
export default function Creative() {
  ...
  const renderDodecahedronIcon = (size: number = miniIconSize) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <path d="M12 2L20 7V17L12 22L4 17V7L12 2Z" stroke="white" strokeWidth="1.5" fill="none" opacity="0.85"/>
      <path d="M8 9L16 9L18 14L12 18L6 14L8 9Z" stroke="white" strokeWidth="1" fill="none" opacity="0.5"/>
  ...
  const renderDodecahedronIconLarge = (size: number = folderIconSize) => (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none">
      <path d="M24 4L40 14V34L24 44L8 34V14L24 4Z" stroke="white" strokeWidth="1.8" fill="none" opacity="0.9"/>
      <path d="M24 9L36 16V32L24 39L12 32V16L24 9Z" stroke="white" strokeWidth="1.2" fill="none" opacity="0.45"/>
      <g stroke="white" strokeWidth="0.8" opacity="0.3">
        <path d="M24 4V16"/><path d="M40 14L30 18"/><path d="M40 34L30 30"/><path d="M24 44V32"/><path d="M8 34L18 30"/><path d="M8 14L18 18"/>
      </g>
      <path d="M18 18L30 18L34 28L24 34L14 28L18 18Z" stroke="white" strokeWidth="1.3" fill="none" opacity="0.65"/>
    ...
    <>
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .transition-bridge.in { opacity: 1; visibility: visible; transition: opacity 0.2s cubic-bezier(0.4, 0, 0.2, 1); }
        .transition-bridge.hold { opacity: 1; visibility: visible; }
        .transition-bridge.out { opacity: 0; visibility: visible; transition: opacity 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
        
        .bridge-spinner { width: 32px; height: 32px; position: relative; }
        .bridge-spinner::before { content: ''; position: absolute; width: 100%; height: 100%; border-radius: 50%; border: 2px solid rgba(255, 255, 255, 0.08); }
        .bridge-spinner::after { content: ''; position: absolute; width: 100%; height: 100%; border-radius: 50%; border: 2px solid transparent; border-top-color: rgba(255, 255, 255, 0.7); animation: spin 0.8s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        
        .creative-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 36px 32px; max-width: 280px; margin: 0 auto; }
        ...
        .folder-icon {
          ...
          box-shadow: 0 4px 8px rgba(0,0,0,0.3), 0 8px 16px rgba(0,0,0,0.25), 0 16px 32px rgba(0,0,0,0.2), 0 32px 64px rgba(0,0,0,0.15);
        }
        ...
        .folder-icon::after {
          ...
          height: 35%;
          background: linear-gradient(180deg, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0.05) 60%, transparent 100%);
        ...
        .folder-icon:active { transform: translateZ(0) scale(0.94) translateY(2px); }
        .folder-wrapper:nth-child(1) .folder-icon { transition-delay: 0ms; }
        .folder-wrapper:nth-child(2) .folder-icon { transition-delay: 100ms; }
        
        .folder-preview { display: grid; grid-template-columns: repeat(2, 1fr); gap: 7px; width: 100px; height: 100px; position: relative; z-index: 5; }
        .folder-preview-4 { grid-template-rows: 1fr 1fr; }
        ...
        /* ═══════════════════════════════════════════════════════════════════════════════ */
        /* STATE OF THE ART - FOLDER OVERLAY - Dark floating, consistent with Work page   */
        /* ═══════════════════════════════════════════════════════════════════════════════ */
        ...
        .folder-cards-grid-3 { grid-template-columns: repeat(3, 1fr); grid-template-rows: 1fr; }
        ...
        /* STATE OF THE ART - White text for dark overlay */
        .folder-card-name {
          font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif;
          font-size: 12px;
          font-weight: 400;
        ...
        .expanded-view.entering { visibility: visible; pointer-events: auto; }
        .expanded-view.active { visibility: visible; pointer-events: auto; }
        ...
        .expanded-view.active .expanded-inner { opacity: 1; transform: translateZ(0) scale(1); transition: opacity 0.4s cubic-bezier(0.32, 0.72, 0, 1) 0.05s, transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) 0.05s; }
        .expanded-view.exiting .expanded-inner { opacity: 0; transform: translateZ(0) scale(0.92); transition: opacity 0.25s ease; }
        ...
        .expanded-view.active .expanded-content { opacity: 1; transform: translateZ(0) scale(1); transition: opacity 0.45s cubic-bezier(0.32, 0.72, 0, 1) 0.12s, transform 0.5s cubic-bezier(0.34, 1.4, 0.64, 1) 0.12s; }
        .expanded-view.exiting .expanded-content { opacity: 0; transform: translateZ(0) scale(0.95); transition: opacity 0.2s ease; }
        
        /* STATE OF THE ART - Floating close button, no border */
        .expanded-close {
          margin-top: 40px;
        ...
        .expanded-close:hover { transform: scale(1.1); }
        .expanded-close svg { color: rgba(255, 255, 255, 0.7); }
        
        .showcase-expanded .expanded-content, .showcase-content { width: 100%; height: 100%; max-width: 400px; max-height: 500px; border-radius: 0; filter: none; }
        ...
        .showcase-app { width: 100%; display: flex; flex-direction: column; align-items: center; gap: 20px; padding: 8px; }
        ...
        .showcase-3d-featured { flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0; transform: scale(0.94); transition: opacity 0.5s ease, transform 0.5s ease; }
        ...
        .showcase-3d-selector-item { cursor: pointer; opacity: 0; transform: scale(0.8); transition: opacity 0.3s ease, transform 0.3s ease; transition-delay: var(--delay, 0s); }
        ...
        .showcase-geometry { position: relative; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; }
        .showcase-geometry-light { position: absolute; top: 15%; left: 50%; transform: translateX(-50%); width: 400px; height: 300px; background: radial-gradient(ellipse at 50% 30%, rgba(255,255,255,0.04) 0%, transparent 70%); pointer-events: none; opacity: 0; transition: opacity 0.8s ease; }
        .showcase-geometry-light.visible { opacity: 1; }
        .showcase-geometry-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 28px; margin-top: -40px; }
        
        .showcase-geometry-item { display: flex; flex-direction: column; align-items: center; gap: 12px; cursor: pointer; opacity: 0; transform: translateY(24px) scale(0.9); transition: opacity 0.25s ease, transform 0.25s ease; }
        .showcase-geometry-grid.visible .showcase-geometry-item { opacity: 1; transform: translateY(0) scale(1); transition: opacity 0.6s ease, transform 0.7s cubic-bezier(0.34, 1.4, 0.64, 1); transition-delay: var(--delay, 0s); }
        
        .showcase-geometry-frame { width: 130px; height: 130px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: radial-gradient(circle, rgba(255,255,255,0.015) 0%, transparent 70%); transition: transform 0.35s ease; }
        .showcase-geometry-frame::before { content: ''; position: absolute; inset: -1px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.04); }
        .showcase-geometry-frame:active { transform: scale(0.94); }
        .showcase-geometry-content { position: relative; z-index: 2; opacity: 0; transition: opacity 0.2s ease; }
        .showcase-geometry-grid.visible .showcase-geometry-content { opacity: 1; transition: opacity 0.5s ease; transition-delay: inherit; }
        ...
        .icons-showcase { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; padding: 12px; }
        .icons-showcase-grid { display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(2, 1fr); gap: 10px; }
        
        .showcase-card { display: flex; flex-direction: column; align-items: center; gap: 5px; opacity: 0; transform: translateY(12px) scale(0.9); transition: opacity 0.4s ease, transform 0.5s cubic-bezier(0.34, 1.4, 0.64, 1); transition-delay: var(--delay, 0s); }
        .icons-showcase-grid.visible .showcase-card { opacity: 1; transform: translateY(0) scale(1); }
        
        .showcase-card-inner { width: 54px; height: 68px; border-radius: 10px; background: linear-gradient(168deg, #ffffff 0%, #f8f7f4 30%, #f2f0eb 70%, #eae8e0 100%); display: flex; align-items: center; justify-content: center; overflow: hidden; box-shadow: 0 1px 2px rgba(0,0,0,0.08), 0 4px 12px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,1); transition: transform 0.25s ease; }
        ...
        .showcase-card-label { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif; font-size: 9px; font-weight: 500; color: rgba(255,255,255,0.55); text-align: center; opacity: 0; transform: translateY(3px); transition: opacity 0.35s ease, transform 0.35s ease; transition-delay: calc(var(--delay, 0s) + 0.15s); }
        .icons-showcase-grid.visible .showcase-card-label { opacity: 1; transform: translateY(0); }
        
        .sub-expanded-view { position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 4000; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; opacity: 0; visibility: hidden; pointer-events: none; }
        ...
        .sub-expanded-bg { position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(5, 5, 5, 0.96); backdrop-filter: blur(40px); }
        .sub-expanded-bg-black { background: #030303 !important; backdrop-filter: none !important; }
        .sub-expanded-inner { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -60%); z-index: 2; display: flex; flex-direction: column; align-items: center; gap: 20px; }
        
        .sub-expanded-header { text-align: center; display: flex; flex-direction: column; gap: 4px; opacity: 0; transform: translateY(-6px); transition: opacity 0.15s ease 0.05s, transform 0.18s ease 0.05s; }
        .sub-expanded-view.active .sub-expanded-header { opacity: 1; transform: translateY(0); }
        .sub-expanded-title { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif; font-size: 28px; font-weight: 600; color: #ffffff; }
        .sub-expanded-subtitle { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif; font-size: 11px; font-weight: 500; color: rgba(255,255,255,0.35); letter-spacing: 0.12em; text-transform: uppercase; }
        
        .sub-expanded-content { position: relative; width: 280px; height: 280px; display: flex; align-items: center; justify-content: center; opacity: 0; transform: scale(0.96); transition: opacity 0.14s ease, transform 0.16s ease; }
        .sub-expanded-view.active .sub-expanded-content { opacity: 1; transform: scale(1); }
        .sub-expanded-content-3d { background: #080808; border-radius: 28px; box-shadow: 0 0 0 1px rgba(255,255,255,0.05), 0 20px 60px rgba(0,0,0,0.5); }
        
        .sub-expanded-glow-white { position: absolute; top: 50%; left: 50%; width: 200px; height: 200px; transform: translate(-50%, -50%); background: radial-gradient(circle, rgba(255,255,255,0.04) 0%, transparent 70%); pointer-events: none; opacity: 0; transition: opacity 0.5s ease 0.2s; }
        .sub-expanded-view.active .sub-expanded-glow-white { opacity: 1; }
        
        /* STATE OF THE ART - Sub expanded close button, no border */
        .sub-expanded-close { width: 44px; height: 44px; background: transparent; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0.6; transition: opacity 0.1s ease, transform 0.1s ease; margin-top: 8px; }
        ...
        .interactive-experience { background: radial-gradient(ellipse at center, #080809 0%, #030304 100%); position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; overflow: hidden !important; touch-action: none !important; }
        .interactive-ambient { position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0; transition: opacity 0.8s ease 0.2s; pointer-events: none; }
        .interactive-experience.active .interactive-ambient { opacity: 1; }
        .interactive-inner { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 16px; padding: 16px; padding-bottom: 60px; position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: 2; overflow: hidden !important; }
        
        .interactive-header { text-align: center; display: flex; flex-direction: column; gap: 2px; opacity: 0; transform: translateY(-10px); transition: opacity 0.4s ease 0.15s, transform 0.5s ease 0.15s; pointer-events: none; }
        .interactive-experience.active .interactive-header { opacity: 1; transform: translateY(0); }
        .interactive-title { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif; font-size: 22px; font-weight: 600; color: #ffffff; }
        .interactive-subtitle { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif; font-size: 10px; font-weight: 500; color: rgba(255,255,255,0.3); letter-spacing: 0.12em; text-transform: uppercase; }
        
        .interactive-content { width: 320px !important; height: 320px !important; border-radius: 24px; overflow: hidden !important; opacity: 0; transform: scale(0.85); transition: opacity 0.5s ease 0.2s, transform 0.6s cubic-bezier(0.34, 1.4, 0.64, 1) 0.2s; }
